...
export async function middleware(req: NextRequest) {
  ...
  console.log(new Date().toISOString(), session?.expires_at);

  const isAccessTokenExpired =
    session && isBefore(session.expires_at, new Date().toISOString());

  console.log({ isAccessTokenExpired });
  // Check on expired of token
  // const needNewToken =
  //   !session?.spotify_access_token && session?.spotify_refresh_token;
  // console.log(needNewToken);
  // if (needNewToken) {
